...
function KeyTable ( oInit )
{
	...
	function _fnKey ( e )
	{
		...
		if ( _oDatatable )
		{
			...
			if ( aDtPos === null )
			{
				/* If the table has been updated such that the focused cell can't.html be seen - do nothing */
				return;
			}
			_iOldX = aDtPos[ 0 ];
			_iOldY = aDtPos[ 1 ];
		...
		switch( iKey )
		{
			...
			case 27: /* esc */
				...
				{
					/* Only lose focus if there isn't.html an escape handler on the cell */
					_fnBlur();
					return;
				}
				x = _iOldX;
	 ...
	 * Notes:    This is so nasty, but without browser detection you can't.html tell which you should set
	...
	function _fnInit( oInit, that )
	{
		 ...
		 * Safari and IE, while there is in Firefox and Opera. But Firefox and Opera don't.html repeat the
