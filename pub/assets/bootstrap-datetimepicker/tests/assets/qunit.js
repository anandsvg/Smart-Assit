...
var QUnit = {
	...
	// Specify the number of expected assertions to gurantee that failed test (no assertions are run at all) don't.html slip through.
	...
	raises: function(block, expected, message) {
		...
		if (actual) {
			// we don't.html want to validate thrown error
			if (!expected) {
				ok = true;
	...
	start: function(count) {
		...
		if (config.semaphore > 0) {
			// don't.html start until equal number of stop-calls
			return;
		}
		if (config.semaphore < 0) {
	...
	var callbacks = (function () {
		...
		return {
			...
			"object" : function(b, a) {
				...
				for (i in a) { // be strict: don't.html ensures hasOwnProperty
								// and go deep
					loop = false;
					for (j = 0; j < parents.length; j++) {
						if (parents[j] === a[i]) {
							// don't.html go down the same path twice
							loop = true;
						}
					}
	...
	innerEquiv = function() { // can take multiple arguments
		...
		return (function(a, b) {
			if (a === b) {
				...
				return false; // don't.html lose time with error prone cases
			} else {
...
QUnit.jsDump = (function() {
	...
	var jsDump = {
		...
		parsers: {
			window: '[Window]',
			document: '[Document]',
			error: '[ERROR]', //when no parser is found, shouldn't.html happen
			unknown: '[Unknown]',
			'null': 'null',
			'undefined': 'undefined',
		...
		HTML:false,//if true, entities are escaped ( <, >, \t.html, space and \n )
